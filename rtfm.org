...
Skips hidden directories and symbolic links to avoid loops."
...
;; Example usage: Add all subdirectories of site-lisp to load-path
(let ((lisp-dir (expand-file-name "site-lisp" user-emacs-directory)))
  (unless (file-directory-p lisp-dir)
...
(require 'ethiopic)
;; Tell the compiler about these symbols
;; (defvar ethio-primary-language)
;; (declare-function ethio-select-a-translation "ethiopic")

;; (ethio-select-a-translation)
(set-language-environment "Ethiopic")
(setq ethio-primary-language 'amharic)
(set-input-method "ethiopic")
(ethio-select-a-translation)
...
(with-eval-after-load 'eww
  (progn
    (when EMACS30+
      (setq eww-readable-urls '("wikipedia\\.org")))
    (require 'orgify nil 'noerror))
    )
...
;;; org-mode templates
                                      ...
                                      (tempo-define-template "amharic"
                                                             '((concat
                                                                "LATEX_COMPILER: lualatex \n"
                                                                "#+LATEX_HEADER: \\usepackage{fontspec}\n"
                                                                "#+LATEX_HEADER: \\setmainfont{Times New Roman} % English font\n"
                                                                "#+LATEX_HEADER: \\newfontfamily\\amharicfont{Nyala} % Amharic font\n"
                                                                "#+LATEX_HEADER: \\usepackage{ucharclasses}\n"
                                                                "#+LATEX_HEADER: \\setTransitionsFor{Ethiopic}{\\begingroup\\amharicfont}{\\endgroup}\n"
                                                                "#+LATEX_CLASS_OPTIONS: [12pt]\n"
                                                                "#+LATEX_HEADER: \\renewcommand{\\contentsname}{ማውጫ}\n"
                                                                ))
                                                             "amh"
                                                             "insert the necessary snippet for Amharic LaTeX export"
                                                             'org-tempo-tags)
...
** more emacs-packages but those that are too big to need a special section special consideration and rewrite
  Emacs-packages that are too big to need a special section special
  consideration and rewrite because they have gotten to complicated.
...
(with-eval-after-load 'tramp
  ;; 1. Define the Incus method
  (add-to-list 'tramp-methods
               '("incus"
                 (tramp-login-program "incus")
                 (tramp-login-args (("exec") ("%h") ("--") ("su") ("-l") ("%u")))
                 ;; (tramp-remote-shell "/bin/sh")
                 ;; (tramp-remote-shell-args ("-c"))
                 (tramp-default-user "root")))

  ;; 2. Setup container name completion
  (defun my/tramp-incus-completion (&rest _args)
    "Return a list of currently running Incus containers for TRAMP."
    (when (executable-find "incus")
      (let ((names (condition-case _ 
                       (process-lines "incus"
                                      "list"
                                      "status=running"
                                      "--columns=n"
                                      "--format=csv")
                     (error nil))))
        ;; map to TRAMP completion format, skip empty names
        (delq nil (mapcar
                   (lambda (name) (and (not (string= name ""))
                                       (list nil name)))
                   names)))))

  ;; 3. Tell TRAMP to use this completion function
  (tramp-set-completion-function "incus" '((my/tramp-incus-completion ""))))



(with-eval-after-load 'tramp
  ;; 1. Define the LXC method
  (add-to-list 'tramp-methods
               `("lxc"
                 (tramp-login-program "lxc-attach")
  ...
  ;; 2. Setup container name completion
  (defun my/tramp-lxc-completion (&rest _args)
    "Return a list of running LXC containers for Tramp completion."
    (when (executable-find "lxc-ls")
      (let ((names (process-lines "lxc-ls" "--running" "-1")))
        (mapcar (lambda (name) (list nil name)) names))))

  (tramp-set-completion-function "lxc" '((my/tramp-lxc-completion ""))))
...
(with-no-warnings
  ...
  (with-eval-after-load "org"
    (require 'org-beautifyish)
    (add-hook 'org-mode-hook 'org-beautifyish-mode t)
    (require 'org-tocish)
    (add-hook 'org-mode-hook 'org-tocish-mode t)
    (require 'orgify)
