...
First, it checks if your version of Emacs supports native compilation
(a feature that speeds up the execution of Emacs Lisp code). If it
does, the code enables deferred compilation. Next, it customizes the
warning settings to suppress certain types of messages. Specifically,
it silences warnings from Flycheck (a popular syntax checker) and from
...
#+begin_src emacs-lisp
  ...
  ;;     (customize-set-variable 'warning-suppress-types '((flycheck syntax-checker) (comp)))))
  ...
  ;;   ;; Only load 'comp if it's available, avoiding errors on Emacs <28
  ;;   (require 'comp nil t) ;; 'nil t' => no error if not found
  ;;   (setq comp-deferred-compilation t)
  ;;   (customize-set-variable 'warning-suppress-types
  ;;                           '((flycheck syntax-checker) (comp))))
...
(add-hook 'after-init-time-hook
          (progn
            (customize-set-variable 'yae-selected-palette 'palenight)
            (load-theme 'yae_simple t)))
